var save_out = {
  ...
  save_scene: function(){
    ...
    // someday this should be detected, rather than manually listes
    containers = [];
    ...
    data_string += "&active_image=" + images.set_array[images.cursor];

    // get the url of the next active image
    data_string += "&next_active_image=" + images.set_array[images.cursor+1];

    //get the style of the background
    data_string += "&bg_style=" + bg_mod.box.attr('style');

    //get the style of the inner background
    data_string += "&inner_bg_style=" + inner_bg_mod.box.attr('style');
