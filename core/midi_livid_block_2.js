...
var block_blinking;
var currently_blinking_button = null;
var blink_on = false;
...
window.setInterval(function(){
  ...
  if(a2[0]==145&&a2[1]==64&&a2[2]==64){/////LIVID BUTTON
    ...
    scenes_helper.activate_scene_by_name('mjp');
  
  }else if(a2[0]==145&&a2[2]==64){/////GRID BUTTONS

    if(currently_blinking_button){
      illuminate_on_block(currently_blinking_button);
    }
    
    clearInterval(block_blinking);
    currently_blinking_button = a2[1];


    scenes_helper.activate_scene_by_name(scenes_for_buttons[a2[1]]);

    block_blinking = setInterval(function(){
      if(blink_on){
        blackout_on_block(a2[1]);
        blink_on = false;
      }else{
        illuminate_on_block(a2[1]);
        blink_on = true;
      }
    },100);
